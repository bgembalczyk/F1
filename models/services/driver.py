...
def extract_count(text: str) -> tuple[int, list[str]]:
...
def parse_season_parts(parts: list[str]) -> list[SeasonRef]:
    if not parts:
        return []
...
def parse_championships(raw: Any) -> DriversChampionships:
    ...
    count, seasons_parts = extract_count(text)
    seasons = [] if count == 0 else parse_season_parts(seasons_parts)
