...
EXCLUDE_DIRS = {'__pycache__', '.git', '.idea', 'node_modules',
                'venv', '.venv', 'tests', 'benchmarks', 'artifacts'}
...
def classify(name: str) -> str:
    if name.endswith('Protocol') or name.endswith('Contract'):
        return 'interface'
    if ('ABC' in name or name.endswith('Base') or name.startswith('Base')
            or 'Mixin' in name):
...
def write(path: Path, content: str) -> None:
    ...
    n_cls = sum(1 for l in content.splitlines()
                if (l.startswith('class ') or l.startswith('abstract class ')
...
p01 = {c for c in p01 if (
    (c.endswith('ABC') or c.endswith('Base') or c.startswith('Base')
     or 'Mixin' in c or c in KEEP_IN_CORE)
    and not c.startswith('Wiki')  # Wiki-prefixed ABCs belong in 02_wiki_html_parsers
)}
write(OUT / 'parsers/01_parser_abc_core.puml',
...
# PARSERS/03 – Scrapers
SCRAPER_ROOTS = ['ScraperLifecycleABC', 'ListScraperContract', 'TableScraperContract',
                 'SectionAwareMixin', 'WikiElementParsingMixin', 'FetchOrchestrationMixin']
p03: set[str] = set()
for r in SCRAPER_ROOTS:
    p03 |= descendants(r, main_comp)
p03 |= {c for c in main_comp if 'Scraper' in c}
p03 |= {'WikiScraper', 'RecursiveSectionParser', 'SectionAdapter',
        'SectionAdapterScraperBase', 'SectionByIdScraperBase',
        'BodyContentAssembler', 'ArticleTablesParser', 'ArticleTablesParserABC',
        'ArticleSectionTablesHtmlParser', 'SeedListTableScraper'} & (main_comp | all_names)
p03 -= p01
write(OUT / 'parsers/03_scrapers.puml',
...
p04 |= {c for c in main_comp if ('SectionParser' in c or
         c.endswith('SubSectionParser') or c.endswith('SubSubSectionParser')
         or c.endswith('SubSubSubSectionParser'))}
p04 -= p01 | p02 | p03
write(OUT / 'parsers/04_section_parsers_domain.puml',
...
p05 |= {c for c in main_comp if 'TableParser' in c
        and c not in ('SectionTablesHtmlParserABC',)}
p05 -= p01 | p02 | p03 | p04
write(OUT / 'parsers/05_table_parsers.puml',
...
write(OUT / 'parsers/06_wiki_section_nodes.puml',
...
# COLUMNS
# ══════════════════════════════════════════════════════════════════════════════
write(OUT / 'columns/01_columns.puml',
      make_puml('Column Hierarchy (BaseColumn)',
                sorted(component_of('BaseColumn'))))

# ══════════════════════════════════════════════════════════════════════════════
# MAPPERS
# ══════════════════════════════════════════════════════════════════════════════
write(OUT / 'mappers/01_mappers.puml',
      make_puml('Mapper Hierarchy (MapperABC)',
                sorted(component_of('MapperABC'))))

# ══════════════════════════════════════════════════════════════════════════════
# RECORDS
# ══════════════════════════════════════════════════════════════════════════════
write(OUT / 'records/01_record_factories.puml',
      make_puml('Record Factories', sorted(component_of('BaseRecordFactory'))))

write(OUT / 'records/02_record_transformers.puml',
      make_puml('Record Transformers', sorted(component_of('RecordTransformer'))))

write(OUT / 'records/03_record_validators.puml',
      make_puml('Record Validators', sorted(component_of('RecordValidator'))))

write(OUT / 'records/04_record_strategies.puml',
      make_puml('Record Split & Assembly Strategies',
                sorted(component_of('SplitRule')
                       | component_of('RecordSplitStrategy')
                       | component_of('RecordAssemblyStrategy')
                       | component_of('BaseRecordAssembler'))))

write(OUT / 'records/05_value_objects_models.puml',
      make_puml('Value Objects & Domain Models',
                sorted(component_of('ValueObject')
...
# SERVICES
# ══════════════════════════════════════════════════════════════════════════════
write(OUT / 'services/01_pipeline_services.puml',
      make_puml('Pipeline & Domain Services',
                sorted(component_of('PipelineService'))))

write(OUT / 'services/02_base_components.puml',
      make_puml('Base Components & Section Extraction Services',
                sorted(component_of('BaseComponent')
                       | component_of('BaseSectionExtractionService'))))

# ══════════════════════════════════════════════════════════════════════════════
# CLASSIFIERS + COMPLETE EXTRACTORS
# ══════════════════════════════════════════════════════════════════════════════
write(OUT / 'components/01_classifiers.puml',
      make_puml('Classifier Hierarchy (ClassifierABC)',
                sorted(component_of('ClassifierABC'))))

write(OUT / 'components/02_complete_extractors.puml',
      make_puml('Complete Data Extractors',
                sorted(component_of('BaseDataExtractor'))))

# ══════════════════════════════════════════════════════════════════════════════
# INFOBOX
# ══════════════════════════════════════════════════════════════════════════════
write(OUT / 'infobox/01_orchestrators.puml',
      make_puml('Infobox Orchestrators & Providers',
                sorted(component_of('InfoboxOrchestratorABC')
...
write(OUT / 'infobox/02_text_utils.puml',
      make_puml('Circuit & Infobox Text Utilities',
                sorted(component_of('InfoboxTextUtils'))))

# ══════════════════════════════════════════════════════════════════════════════
# INFRASTRUCTURE
# ══════════════════════════════════════════════════════════════════════════════
write(OUT / 'infrastructure/01_errors.puml',
      make_puml('Error Hierarchy',
                sorted(component_of('ScraperError')
                       | component_of('RequestError'))))

write(OUT / 'infrastructure/02_http_cache.puml',
      make_puml('HTTP Client, Cache & Rate Limiter',
                sorted(component_of('BaseHttpClient')
...
write(OUT / 'infrastructure/03_layers.puml',
      make_puml('Layers, Executors & Job Runners',
                sorted(component_of('BaseExecutor')
...
if misc_classes:
    write(OUT / 'misc/01_misc_small.puml',
