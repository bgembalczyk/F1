...
def _run_mypy(repo_dir: Path) -> tuple[int, str]:
    proc = subprocess.run(  # noqa: S603  # nosec B603 -- uruchomienie zaufanego `python -m mypy`
...
def _git(*args: str) -> None:
    subprocess.run(  # noqa: S603  # nosec B603 -- zaufane wywołanie lokalnego `git`
        [GIT_BIN, *args],
        check=True,
        text=True,
        capture_output=True,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="mypy-base-") as temp_dir:
        ...
        try:
            ...
            subprocess.run(  # noqa: S603  # nosec B603 -- zaufane wywołanie lokalnego `git worktree remove`
