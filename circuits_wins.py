...
data = results.merge(races, on="raceId", how="left").merge(drivers, on="driverId", how="left").merge(constructors, on="constructorId", how="left").merge(circuits, on="circuitId", how="left", suffixes=("", "_circuit"))

winners = data[data["positionOrder"] == 1]

# Zliczanie zwycięstw na torach
circuit_wins = winners.groupby(["circuitId", "name_circuit", "driverId", "forename", "surname"]).size().reset_index(name="wins")
