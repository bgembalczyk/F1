...
class SourceTransformStrategy:
    ...
    excluded_source_pattern: re.Pattern[str] | None = None
    ...
    def supports(self, domain: str, source_name: str) -> bool:
        ...
        if self.source_pattern is not None and self.source_pattern.fullmatch(source_name) is None:
            return False
        if (
            self.excluded_source_pattern is not None
            and self.excluded_source_pattern.fullmatch(source_name) is not None
        ):
            return False
        return True
...
def _apply_strategy_subset(
    *,
    ...
    transformed = dict(record)
    for strategy in TRANSFORM_STRATEGIES:
        if strategy.name not in strategy_names:
            continue
        if not strategy.supports(domain, canonical_source_name):
            continue
        transformed = strategy.apply(transformed)
    return transformed
...
TRANSFORM_STRATEGIES: tuple[SourceTransformStrategy, ...] = (
    ...
    SourceTransformStrategy(
        ...
        domains=frozenset(CHASSIS_CONSTRUCTOR_DOMAINS),
        excluded_source_pattern=F1_CONSTRUCTORS_YEAR_SOURCE_PATTERN,
...
LEGACY_HANDLER_STRATEGY_NAMES: dict[str, frozenset[str]] = {
    "_tyre_manufacturers_handler": frozenset({"tyre_manufacturers"}),
    "_constructor_domain_handler": frozenset(
        {
            "constructor_indianapolis_only",
            "constructor_former",
            "constructor_base",
            "constructor_year_source",
        },
    ),
    "_circuits_domain_handler": frozenset({"circuits_base"}),
    "_engines_domain_handler": frozenset(
        {"engines_indianapolis_only", "engines_manufacturers"},
    ),
    "_grands_prix_domain_handler": frozenset({"grands_prix_base"}),
    "_teams_domain_handler": frozenset(
        {
            "teams_constructor_year_alias",
            "teams_sponsorship_liveries",
            "teams_privateer",
        },
    ),
    "_drivers_domain_handler": frozenset(
        {"drivers_f1", "drivers_female", "drivers_fatalities"},
    ),
    "_races_domain_handler": frozenset(
        {
            "races_world_championship",
            "races_non_championship",
            "races_red_flag",
        },
    ),
}
...
def _tyre_manufacturers_handler(
    ...
    return _apply_strategy_subset(
        domain=domain,
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_tyre_manufacturers_handler"],
    )
...
def _constructor_domain_handler(
    ...
    return _apply_strategy_subset(
        ...
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_constructor_domain_handler"],
    )
...
def _circuits_domain_handler(
    ...
    return _apply_strategy_subset(
        ...
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_circuits_domain_handler"],
    )
...
def _engines_domain_handler(
    ...
    return _apply_strategy_subset(
        ...
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_engines_domain_handler"],
    )
...
def _grands_prix_domain_handler(
    ...
    return _apply_strategy_subset(
        ...
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_grands_prix_domain_handler"],
    )
...
def _teams_domain_handler(
    ...
    return _apply_strategy_subset(
        ...
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_teams_domain_handler"],
    )
...
def _drivers_domain_handler(
    ...
    return _apply_strategy_subset(
        ...
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_drivers_domain_handler"],
    )
...
def _races_domain_handler(
    ...
    return _apply_strategy_subset(
        ...
        source_name=source_name,
        record=record,
        strategy_names=LEGACY_HANDLER_STRATEGY_NAMES["_races_domain_handler"],
    )


DOMAIN_LEGACY_HANDLER_BY_NAME: dict[str, RecordTransformHandler] = {
    "constructors": _constructor_domain_handler,
    "constructor": _constructor_domain_handler,
    "chassis": _constructor_domain_handler,
    "circuits": _circuits_domain_handler,
    "engines": _engines_domain_handler,
    "grands_prix": _grands_prix_domain_handler,
    "teams": _teams_domain_handler,
    "drivers": _drivers_domain_handler,
    "races": _races_domain_handler,
}
...
def _resolve_record_transform_handlers(
    ...
    if resolve_list_filename(source_name) == TYRE_MANUFACTURERS_SOURCE:
        handlers.append(_tyre_manufacturers_handler)
    domain_handler = DOMAIN_LEGACY_HANDLER_BY_NAME.get(domain)
    if domain_handler is not None:
        handlers.append(domain_handler)
