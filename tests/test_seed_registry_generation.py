...
def test_generated_seed_registry_matches_legacy_contract() -> None:
    ...
    assert (
        by_seed["drivers"].default_output_path == "raw/drivers/seeds/complete_drivers"
    )
    ...
    assert (
        by_seed["constructors"].legacy_output_path
    ...
    assert (
        by_seed["circuits"].default_output_path
        == "raw/circuits/seeds/complete_circuits"
    )
    assert by_seed["circuits"].legacy_output_path == "circuits/complete_circuits"
    assert (
        by_seed["seasons"].default_output_path == "raw/seasons/seeds/complete_seasons"
    )
