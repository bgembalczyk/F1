...
def test_discovery_validation_fails_for_runner_without_metadata(monkeypatch: pytest.MonkeyPatch) -> None:
    fake_module = ModuleType("scrapers.fake.discovery_test")

    class NewRunner:  # noqa: D401
        pass

    NewRunner.__name__ = "NewRunner"
    NewRunner.__module__ = fake_module.__name__
    setattr(fake_module, "NewRunner", NewRunner)

    monkeypatch.setattr(discovery, "_iter_discovery_module_names", lambda: (fake_module.__name__,))
...
def test_seed_registry_falls_back_to_explicit_when_discovery_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
