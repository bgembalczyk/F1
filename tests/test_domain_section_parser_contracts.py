...
def test_constructor_section_parser_contract() -> None:
    ...
    result = parser.parse(BeautifulSoup('<table class="wikitable"><tr><th>Year</th></tr><tr><td>1950</td></tr></table>', 'html.parser'))
...
def test_driver_section_parser_contract() -> None:
    raw = DriverResultsSectionParser(options=ScraperOptions(include_urls=False), url='https://example.com/driver')
...
def test_season_section_parser_contract() -> None:
    ...
    result = parser.parse(BeautifulSoup('<ul><li>Rules update</li></ul>', 'html.parser'))
    assert tuple(asdict(result).keys()) == CONTRACT_KEYS


def test_grand_prix_section_parser_contract() -> None:
    parser = GrandPrixByYearSectionParser(url='https://example.com/gp', include_urls=False, normalize_empty_values=True)
