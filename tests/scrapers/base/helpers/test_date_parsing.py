...
def test_parse_date_with_category_marker_empty_returns_none() -> None:
    assert parse_date_with_category_marker(_ctx(""), "#") is None
    assert parse_date_with_category_marker(_ctx(None), "#") is None


def test_parse_date_with_category_marker_strips_marker() -> None:
    # Should parse "2024-03-15" after stripping "#"
    result = parse_date_with_category_marker(_ctx("#2024-03-15"), "#")
    assert result is not None


def test_parse_date_with_category_marker_returns_string() -> None:
    result = parse_date_with_category_marker(_ctx("2020-01-01"), "#")
    assert isinstance(result, str) or result is None


def test_parse_formula_category_none_text_returns_none() -> None:
    assert parse_formula_category(_ctx(None, raw_text=None), "#") is None
    assert parse_formula_category(_ctx("", raw_text=""), "#") is None


def test_parse_formula_category_with_marker_returns_f2() -> None:
    # raw_text contains "#" -> F2
    result = parse_formula_category(_ctx("2024 race", raw_text="2024 race #"), "#")
    assert result == "F2"
