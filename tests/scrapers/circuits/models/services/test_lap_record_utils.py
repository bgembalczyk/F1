...
def test_extract_year_from_event_dict_text() -> None:
    ...
    assert utils.extract_year_from_event(rec) == "1963"
...
def test_extract_year_from_year_field() -> None:
    rec = {"year": "2019"}
    assert utils.extract_year(rec) == "2019"
...
def test_normalize_lap_record_entity_plain() -> None:
    assert utils.normalize_lap_record_entity("Lewis Hamilton") == "lewis hamilton"
...
def test_normalize_lap_record_entity_with_sanitizer() -> None:
    ...
    result = utils.normalize_lap_record_entity("Lewis Hamilton", sanitizer=sanitize)
    assert "ham" in result


def test_normalize_lap_record_entity_empty() -> None:
    assert utils.normalize_lap_record_entity("") == ""
    assert utils.normalize_lap_record_entity(None) == ""
...
def test_parse_time_from_time_seconds() -> None:
    ...
    assert utils.parse_lap_record_time_from_record(rec) == pytest.approx(83.456)
...
def test_parse_time_from_time_string() -> None:
    rec = {"time": "1:23.456"}
    result = utils.parse_lap_record_time_from_record(rec)
    assert result is not None
    assert result == pytest.approx(83.456, rel=1e-3)


def test_parse_time_none() -> None:
    assert utils.parse_lap_record_time_from_record({}) is None
...
def test_has_meaningful_value(candidate, expected) -> None:
    assert utils.has_meaningful_value(candidate) == expected
...
def test_select_best_field_no_records() -> None:
    assert utils.select_best_field_with_url([], "driver") is None


def test_select_best_field_single() -> None:
    records = [{"driver": {"text": "Hamilton", "url": None}}]
    result = utils.select_best_field_with_url(records, "driver")
...
def test_select_best_field_multiple_field_names() -> None:
    ...
    result = utils.select_best_field_with_url(records, "vehicle", "car")
...
def test_build_lap_record_key_basic() -> None:
    ...
    key = utils.build_lap_record_key(rec)
...
def test_build_lap_record_key_returns_none_when_missing_driver() -> None:
    ...
    assert utils.build_lap_record_key(rec) is None
...
def test_build_lap_record_key_custom_year_extractor() -> None:
    ...
    key = utils.build_lap_record_key(rec, year_extractor=utils.extract_year_from_event)
    assert key is not None
    assert "2020" in key
...
def test_build_lap_record_key_custom_time_key_factory() -> None:
    ...
    key = utils.build_lap_record_key(rec, time_key_factory=lambda t: int(t * 1000))
    assert key is not None
...
def test_build_lap_record_key_custom_vehicle_getter() -> None:
    ...
    key = utils.build_lap_record_key(rec, vehicle_getter=lambda r: r.get("car"))
    assert key is not None
