...
PRIVATE_MEMBER_ACCESS = re.compile(r"\.\_[A-Za-z]")
...
JUSTIFICATION_TAG = "PRIVATE-API-JUSTIFIED"


def integration_tests() -> list[Path]:
    tests_dir = Path(__file__).parent
    files = sorted(tests_dir.glob("test_*.py"))
    return [
        path
        for path in files
        if any(pattern in path.name for pattern in INTEGRATION_PATTERNS)
    ]
...
def test_integration_tests_do_not_touch_private_api_without_justification() -> None:
    ...
    for path in integration_tests():
        for line_no, line in enumerate(
            ...
            if not PRIVATE_MEMBER_ACCESS.search(line):
                continue
            if JUSTIFICATION_TAG in line:
                continue
    ...
    assert not violations, (
        ...
        f"tag '{JUSTIFICATION_TAG}' on the same line. Violations: {violations}"
    )
