...
from scrapers.seasons.parsers import constants
...
class DomainParsingPolicy:
    ...
    def resolve_engine_config(self, season_year: int | None) -> dict[str, Any] | None:
        ...
        if (
            ...
            and constants.ENGINE_V10_START_YEAR <= season_year <= constants.ENGINE_V10_END_YEAR
        ):
            return {"displacement_l": 3.0, "layout": "V", "cylinders": 10}
        return None

    def should_normalize_entry_numbers(self, season_year: int | None) -> bool:
        return season_year is not None and season_year < constants.PRE_2007_NORMALIZATION_CUTOFF
