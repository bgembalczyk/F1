...
def _run_list_scraper_adapter(module_path: str) -> Callable[..., None]:
    if module_path == "scrapers.circuits.entrypoint:run_list_scraper":
        from scrapers.circuits.entrypoint import run_list_scraper

        return run_list_scraper
    if module_path == "scrapers.constructors.entrypoint:run_list_scraper":
        from scrapers.constructors.entrypoint import run_list_scraper

        return run_list_scraper
    if module_path == "scrapers.drivers.entrypoint:run_list_scraper":
        from scrapers.drivers.entrypoint import run_list_scraper

        return run_list_scraper
    if module_path == "scrapers.grands_prix.entrypoint:run_list_scraper":
        from scrapers.grands_prix.entrypoint import run_list_scraper

        return run_list_scraper
    if module_path == "scrapers.points.sprint_qualifying_points:run_list_scraper":
        from scrapers.points.sprint_qualifying_points import run_list_scraper

        return run_list_scraper
    if module_path == "scrapers.points.points_scoring_systems_history:run_list_scraper":
        from scrapers.points.points_scoring_systems_history import run_list_scraper

        return run_list_scraper
    if module_path == "scrapers.points.shortened_race_points:run_list_scraper":
        from scrapers.points.shortened_race_points import run_list_scraper

        return run_list_scraper
    if module_path == "scrapers.seasons.entrypoint:run_list_scraper":
        from scrapers.seasons.entrypoint import run_list_scraper

        return run_list_scraper
    return _import_target(module_path)


def _resolve_target(path: str) -> Callable[..., None]:
    return _run_list_scraper_adapter(path)
...
def _run_and_export_target(
    ...
    def _target(*, run_config: RunConfig) -> None:
        scraper_cls = _resolve_target(class_path)
...
def _run_export_complete(
    ...
    *,
    ...
    def _target() -> None:
        export_fn = _resolve_target(path)
