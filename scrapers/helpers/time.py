...
def parse_with_formats(base: str, formats: tuple[str, ...]) -> datetime | None:
    for fmt in formats:
        parsed = None
        try:
...
def parse_date_iso(base: str) -> str | None:
    parsed_date = parse_with_formats(base, tuple(constants.DATE_FORMATS))
    if parsed_date is not None:
        return parsed_date.date().isoformat()

    parsed_month = parse_with_formats(base, ("%B %Y", "%b %Y"))
