...
from scrapers.parsers.sponsorship_liveries.splitters.record.protocols import SplitRule
from scrapers.parsers.sponsorship_liveries.splitters.record.rules import HasMultipleSeasonsRule
from scrapers.parsers.sponsorship_liveries.splitters.record.rules import HasYearSpecificColoursRule
from scrapers.parsers.sponsorship_liveries.splitters.record.rules import HasYearSpecificSponsorsRule
...
class SeasonSplitStrategy:
    ...
    def _build_base_colour_scoped_records(
        ...
        base_record = {**raw_record, "season": base_seasons}
        for key in COLOUR_KEYS:
            if key in raw_record:
    ...
    def _build_year_scoped_colour_records(
        ...
        for years in SeasonSplitStrategy._unique_year_sets(colour_year_sets):
            ...
            if not scoped_seasons:
                continue
            scoped_record = {**raw_record, "season": scoped_seasons}
            for key in COLOUR_KEYS:
                if key in raw_record:
