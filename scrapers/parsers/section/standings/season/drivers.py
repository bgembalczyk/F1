...
class SeasonDriversStandingsSectionParser(SectionParser):
    def __init__(self, parser: SeasonStandingsService, season_year: int | None) -> None:
        self._parser = parser
        self._season_year = season_year

    def parse(self, fragment: BeautifulSoup) -> SectionParseResult:
        records = self._parser.parse_drivers(fragment, self._season_year)
