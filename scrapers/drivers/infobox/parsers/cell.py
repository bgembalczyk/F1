            ...
            if season_links:
                        ...
                        for season_link in season_links:
                            ...
                            if season_text:
                                ...
                                if season_tag:
                                    ...
                                    while (
                                        ...
                                        if next_elem:
                                            ...
                                            if isinstance(next_elem, Tag):
                                                small_in_sibling = next_elem.find(
                                                    "small"
                                                )
                                                if small_in_sibling:
                                                    # Check if there's a season link before this small
                                                    for a_tag in next_elem.find_all(
                                                        "a"
                                                    ):
                                                        a_text = clean_infobox_text(
                                    ...
                                    if found_small:
                                        class_links = (
                                            self._link_extractor.extract_links(
                                                found_small
                                            )
                                        )
                                        if class_links:
                                            ...
                                            is_valid = not self._is_season_like_text(
                                                class_text
                                            )
                                            # Check if it duplicates the current season
                                            if is_valid and (
                                                season_text == class_text
                                                or season_url == class_url
                                            ):
                                                is_valid = False
                ...
                if " in " in text:
                    ...
                    if paren_match:
                        ...
                        # Extract all years
                        years = [
                            int(y) for y in re.findall(r"\b(\d{4})\b", paren_content)
                        ]
                        if years:
                            result["seasons"] = years
                    ...
                    for year_span in year_spans:
                        ...
                        if self._is_element_before(licence_tag, year_span):
                            ...
                            if not has_licence_between:
                                ...
                                licence_entry["years"] = self._parse_licence_years(
                                    year_text
                                )
                                break
