...
class _ConstructorPartParser(Protocol):
    def __init__(self, index: int) -> None: ...

    def parse(self, ctx: ColumnContext) -> dict[str, object] | None: ...


class BaseConstructorColumn(BaseColumn):
    part_parser_cls: type[_ConstructorPartParser]
    ...
    def _parse_constructor_data(self, ctx: ColumnContext) -> dict[str, object]:
        ...
        chassis = self.part_parser_cls(0).parse(ctx)
        engine = self.part_parser_cls(1).parse(ctx)
        data = {}
        if chassis is not None:
            data["chassis_constructor"] = chassis
        if engine is not None:
