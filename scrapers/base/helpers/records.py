...
def build_record_key(rec: dict[str, Any]) -> tuple | None:
    ...
    return (driver_txt, vehicle_txt, year, round(float(time_sec), 6))
...
def merge_two_records(base: dict[str, Any], extra: dict[str, Any]) -> dict[str, Any]:
    ...
    time -> zawsze float sekundy, series -> series, date/year -> najlepsze.
    """
...
def merge_race_lap_records(records: list[Any]) -> list[dict[str, Any]]:
    """
    Łączy duplikujące się rekordy (infobox + tabela) w jeden bogaty rekord.

    Przyjmuje listę dictów, ale toleruje LapRecord w wejściu.
